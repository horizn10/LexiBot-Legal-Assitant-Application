...
import '../../models/bns_model.dart';
import '../../providers/language_provider.dart';
import '../../utils/number_utils.dart';

class BnsSectionListPage extends StatefulWidget {
  final String chapterNumber;
  final String? sectionNumber;

  const BnsSectionListPage(
      {super.key, required this.chapterNumber, this.sectionNumber});

  @override
  _BnsSectionListPageState createState() => _BnsSectionListPageState();
}

class _BnsSectionListPageState extends State<BnsSectionListPage> {
  BnsData? bnsData;
  String? errorMessage;
  ...
  Future<void> loadData() async {
    try {
      ...
      String jsonFile = 'Dataset/BNS_master.json';
      if (lang.currentLanguage == 'hi') {
        jsonFile = 'Dataset/BNS_master_hindi.json';
      } else if (lang.currentLanguage == 'ne') {
        jsonFile = 'Dataset/BNS_master_nepali.json';
      }
      final String jsonString = await rootBundle.loadString(jsonFile);
      final List<dynamic> jsonData = json.decode(jsonString);
      setState(() {
        bnsData = BnsData.fromJson({'chapters': jsonData});
        errorMessage = null;
      });
  ...
  Widget build(BuildContext context) {
    ...
    if (bnsData == null) {
      return Scaffold(
        appBar: AppBar(
          title: Text(
            '${strings['section_list']} - ${strings['bns']} ${strings['chapter']} ${convertToLocalizedNumbers(widget.chapterNumber, lang.currentLanguage)}',
    ...
    final chapter = bnsData!.chapters.firstWhere(
