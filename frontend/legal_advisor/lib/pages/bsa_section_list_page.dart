...
import '../../models/bsa_model.dart';
import '../../providers/language_provider.dart';
import '../../utils/number_utils.dart';

class BsaSectionListPage extends StatefulWidget {
  final String chapterNumber;
  final String? sectionNumber;

  const BsaSectionListPage(
      {super.key, required this.chapterNumber, this.sectionNumber});

  @override
  _BsaSectionListPageState createState() => _BsaSectionListPageState();
}

class _BsaSectionListPageState extends State<BsaSectionListPage> {
  BsaData? bsaData;
  String? errorMessage;
  ...
  Future<void> loadData() async {
    try {
      ...
      String jsonFile = 'Dataset/BSA_master.json';
      if (lang.currentLanguage == 'hi') {
        jsonFile = 'Dataset/BSA_master_hindi.json';
      } else if (lang.currentLanguage == 'ne') {
        jsonFile = 'Dataset/BSA_master_nepali.json';
      }
      final String jsonString = await rootBundle.loadString(jsonFile);
      final List<dynamic> jsonData = json.decode(jsonString);
      setState(() {
        bsaData = BsaData.fromJson({'chapters': jsonData});
        errorMessage = null;
      });
  ...
  Widget build(BuildContext context) {
    ...
    if (bsaData == null) {
      return Scaffold(
        appBar: AppBar(
          title: Text(
            '${strings['section_list']} - ${strings['bsa']} ${strings['chapter']} ${convertToLocalizedNumbers(widget.chapterNumber, lang.currentLanguage)}',
    ...
    final chapter = bsaData!.chapters.firstWhere(
